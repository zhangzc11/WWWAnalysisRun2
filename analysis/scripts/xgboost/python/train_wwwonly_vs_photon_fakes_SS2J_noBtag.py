...
test_name = 'xgb_wwwonly_vs_photon_fakes_SS2J_noBtag'
...
#signal
signalFileName =  dataDir + 'www_SS2J.root' 
...
#bkg
bkgFileName    = dataDir + 'photon_fakes_SS2J.root'
...
#                ['jet3BtagScore', 'jet3Btag', 'jet3 Btag', 100, 0, 1],
	     	['Mljmin', 'Mljmin', '$m_{lj}^{min} (GeV)$', 100, 0, 500],
	     	['DRljmin', 'DRljmin', '$\Delta R_{lj}^{min}$', 100, 0, 6.0],
...
print("www sample size: "+str(len(df_signal.values)))
...
for idx in range(len(variables)-1):
        ...
        plt.hist(df_signal[df_signal[variables[idx][0]] > -999][variables[idx][0]], density=True, alpha=1.0, histtype="step", lw=3, label="www", bins=variables[idx][3], range=(variables[idx][4], variables[idx][5]))
        plt.hist(df_bkg[df_bkg[variables[idx][0]] > -999][variables[idx][0]], density=True, alpha=1.0, histtype="step", lw=3, label="photon/fakes", bins=variables[idx][3], range=(variables[idx][4], variables[idx][5]))
...
ax = f.add_subplot(111)
plt.subplots_adjust(top=0.9, bottom=0.15, left=0.15, right=0.95)
plt.hist(disc_signal, density=True, bins=100, alpha=1.0, histtype="step", lw=2, label="www - test")
plt.hist(disc_signal_train, density=True, bins=100, alpha=1.0, histtype="step", lw=2, label="www  - train")
plt.hist(disc_bkg, density=True, bins=100, alpha=1.0, histtype="step", lw=2, label="photon/fakes - test")
plt.hist(disc_bkg_train, density=True, bins=100, alpha=1.0, histtype="step", lw=2, label="photon/fakes - train")
plt.yscale("log")
plt.xlim([0.0, 1.0])
